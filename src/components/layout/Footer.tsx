        ...
        <div className='grid gap-12 lg:grid-cols-[1.4fr_repeat(3,1fr)]'>
          ...
          {footerLinkGroups.map((group) => (
            <div key={group.title} className='space-y-5'>
              <h3 className='text-sm font-semibold text-slate-950'>{t(`footer.groups.${group.title}.title`)}</h3>
              <ul className='space-y-3'>
                {group.links.map((link) => (
                  <li key={link.label}>
                    <Link to={link.href} className='text-sm text-slate-500 transition-colors hover:text-violet-700'>
                      {t(`footer.groups.${group.title}.links.${link.label}`)}
                    </Link>
                  </li>
                ))}
              </ul>
        ...
        <Container className='flex flex-col gap-4 py-5 text-sm text-slate-400 sm:flex-row sm:items-center sm:justify-between'>
          ...
          <div className='flex flex-wrap items-center gap-5'>
            <span className='inline-flex items-center gap-1.5'>
              ...
              {t(`footer.language.${currentLanguage}`)}
            </span>
            <span>{t('footer.jobsOnline', { count: 15204 })}</span>
          </div>
        </Container>
      </div>
    </footer>
